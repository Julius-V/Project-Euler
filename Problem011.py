...
def largest_product(g, nr, nc, wd, n):
    ...
    for c in range(1, nc + 1):
        for r in range(1, nr + 1):
            if c >= wd:
                largest = max(largest, grid_prod(g, nc, wd, r, c, n, 1))
            if c >= n & nr - r >= n - 1:
                largest = max(largest, grid_prod(g, nc, wd, r, c, n, 2))
            if nr - r >= n - 1:
                largest = max(largest, grid_prod(g, nc, wd, r, c, n, 3))
            if c <= nc - n + 1 & nr - r >= n - 1:
                largest = max(largest, grid_prod(g, nc, wd, r, c, n, 4))
            if c <= nc - n + 1:
...
print(largest_product(grid, 20, 20, 3, 4))
